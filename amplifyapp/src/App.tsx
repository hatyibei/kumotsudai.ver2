...
import { AlertCircle, Shield, Zap, CheckCircle } from 'lucide-react';
...
const OptimizedAppRouter: React.FC = () => {
  ...
  const performanceMonitor = useMemo(() => PerformanceMonitoringService.getInstance(), []);
  const securityMonitor = useMemo(() => SecurityMonitoringService.getInstance(), []);
  ...
  const config = useMemo(() => {
    try {
      ...
      if (typeof configManager.optimizePerformanceThresholds === 'function') {
        configManager.optimizePerformanceThresholds(150, 200); // より現実的な目標値
      }
      
      // デバイス性能に基づく動的調整
      if (typeof configManager.adjustThresholdsForDevice === 'function') {
        configManager.adjustThresholdsForDevice();
      }
      
      return configManager;
    } catch (error) {
      console.warn('設定管理サービスの初期化に失敗しました:', error);
      // フォールバック実装（より現実的な閾値）
      return {
  ...
  const optimizedInitialization = useCallback(async () => {
    // 【最適化】測定開始を条件付きで実行
    const shouldMeasurePerformance = config.environment === 'development';
    ...
    try {
      logger.info('🚀 最適化された初期化開始', {
        ...
        measurement: shouldMeasurePerformance ? 'enabled' : 'disabled'
      });
      ...
      const [
        ...
        Promise.resolve(securityMonitor.checkContentSecurityPolicy()),
        // ネットワーク状態確認
        Promise.resolve(typeof navigator !== 'undefined' ? navigator.onLine : true),
      ...
      if (config.environment === 'development') {
        logger.debug(theme === 'autumn' ? '秋テーマ初期化完了' : '冬テーマ初期化完了');
      }
      ...
      if (shouldMeasurePerformance && initMeasurement) {
        const duration = performanceMonitor.endMeasurement(initMeasurement);
        ...
        logger.info('✅ 最適化された初期化完了', {
          ...
          performanceGrade: duration <= 100 ? 'Excellent' : duration <= 200 ? 'Good' : 'Acceptable'
        });
      }
  ...
  }, [performanceMonitor, securityMonitor, logger, theme, config]);
  ...
  const renderScreen = useCallback(() => {
    const themeClass = theme === 'autumn' ? 'autumn-theme' : 'winter-theme';
    const commonProps = {
    ...
    switch (currentScreen) {
      ...
      case 'main':
        screenComponent = (
          <div {...commonProps} aria-label="メインアプリケーション">
            <MainApp />
          </div>
        );
        break;
      ...
      case 'userProfile':
        screenComponent = (
          <div {...commonProps} aria-label="ユーザープロフィール">
            <UserProfileScreen />
          </div>
        );
        break;
      ...
      default:
        ...
        if (config.environment === 'development') {
          logger.warn('未知の画面が指定されました', { currentScreen });
        }
    }
    ...
    if (shouldMeasureRender && renderMeasurement) {
      ...
      requestAnimationFrame(() => {
        setTimeout(() => {
          try {
            const duration = performanceMonitor.endMeasurement(renderMeasurement!);
            
            // 閾値を超えた場合のみログ出力（ノイズ削減）
            if (duration > config.performanceThresholds.warning) {
              logger.debug(`画面レンダリング: ${duration.toFixed(0)}ms`, {
                screen: currentScreen,
                target: '200ms以下',
                threshold: config.performanceThresholds.warning,
                grade: duration <= 300 ? 'Acceptable' : 'Needs optimization'
              });
            }
          } catch (error) {
            // 測定エラーは警告レベルで記録（重要度低）
  ...
  }, [currentScreen, theme, performanceMonitor, logger, config]);
    ...
    const handleKeyboardShortcuts = (e: KeyboardEvent) => {
      try {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === 'k' && !e.shiftKey) {
          e.preventDefault();
          const searchInput = document.querySelector('input[placeholder*="探求"]') as HTMLInputElement;
          if (searchInput) {
            searchInput.focus();
            if (config.environment === 'development') {
        ...
        if (config.environment === 'development' && (e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'K') {
          e.preventDefault();
          console.log('🔧 開発者情報:', {
            systemStatus,
            performanceSummary: performanceMonitor.getPerformanceSummary(),
            securitySummary: securityMonitor.getSecuritySummary(),
            configDebug: config.getDebugInfo ? config.getDebugInfo() : {}
          });
        }
        
        // Ctrl+Shift+P でパフォーマンス最適化 (開発環境のみ)
        if (config.environment === 'development' && (e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'P') {
          e.preventDefault();
          if (typeof config.optimizePerformanceThresholds === 'function') {
            config.optimizePerformanceThresholds(100, 150); // より厳しい目標
            toast.info('🚀 パフォーマンス閾値を最適化しました', {
        ...
        if (config.environment === 'development' && (e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'D') {
          e.preventDefault();
          if (typeof config.adjustThresholdsForDevice === 'function') {
            config.adjustThresholdsForDevice();
            toast.info('🔧 デバイス性能に基づいて閾値を調整しました', {
        ...
        // F5 での再読み込み確認 (本番環境)
        if (config.environment === 'production' && e.key === 'F5') {
          e.preventDefault();
          if (confirm('ページを再読み込みしますか？未保存の変更は失われます。')) {
            window.location.reload();
          }
        }
      } catch (error) {
        console.warn('キーボードショートカット処理中にエラーが発生しました:', error);
      }
    };

    document.addEventListener('keydown', handleKeyboardShortcuts);
    return () => document.removeEventListener('keydown', handleKeyboardShortcuts);
  }, [config, systemStatus, performanceMonitor, securityMonitor, logger]);
  ...
  useEffect(() => {
    try {
      ...
      const screenName = screenNames[currentScreen as keyof typeof screenNames] || currentScreen;
      ...
      console.warn('ARIA ライブリージョン更新中にエラーが発生しました:', error);
    }
  ...
  useEffect(() => {
    try {
      ...
      const newTitle = titles[currentScreen as keyof typeof titles] || '供物台 - 神秘の森';
      document.title = newTitle;
      
      // 【最適化】メタデータ更新の条件分岐
      const description = document.querySelector('meta[name="description"]');
      if (description && description.getAttribute('content') !== '供物台は日本の神秘的な森をテーマにした投稿・評価アプリケーションです。') {
        description.setAttribute('content', '供物台は日本の神秘的な森をテーマにした投稿・評価アプリケーションです。');
      }
    } catch (error) {
      console.warn('SEOタイトル更新中にエラーが発生しました:', error);
    }
  }, [currentScreen]);
        ...
        <div className="fixed bottom-4 left-4 z-50 bg-card border rounded-lg p-3 text-xs shadow-lg">
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-1">
              <Shield className={`w-3 h-3 ${systemStatus.security ? 'text-green-500' : 'text-red-500'}`} />
              <span className="text-[10px]">SEC</span>
            </div>
            <div className="flex items-center space-x-1">
              <Zap className={`w-3 h-3 ${systemStatus.performance ? 'text-green-500' : 'text-red-500'}`} />
              <span className="text-[10px]">PERF</span>
            </div>
            <div className="flex items-center space-x-1">
              <div className={`w-2 h-2 rounded-full ${systemStatus.accessibility ? 'bg-green-500' : 'bg-red-500'}`} />
              <span className="text-[10px]">A11Y</span>
            </div>
            <div className="flex items-center space-x-1">
              <div className={`w-2 h-2 rounded-full ${systemStatus.network ? 'bg-green-500' : 'bg-red-500'}`} />
              <span className="text-[10px]">NET</span>
            </div>
          </div>
          <div className="text-[10px] text-muted-foreground mt-1">
            閾値: {config.performanceThresholds.warning}ms / {config.performanceThresholds.error}ms
          </div>
  ...
  useEffect(() => {
    ...
    const initializeServices = async () => {
      try {
        ...
        if (shouldMeasure) {
          ...
          performanceMonitor.startMeasurement(appMountMeasurement);
        }
        
        // 全サービスを並列初期化
        const [config, logger, securityMonitor] = await Promise.all([
          Promise.resolve(AppConfigManager.getInstance()),
          Promise.resolve(LoggerService.getInstance()),
          Promise.resolve(SecurityMonitoringService.getInstance())
        ]);
        ...
        if (shouldMeasure && appMountMeasurement) {
          const duration = performanceMonitor.endMeasurement(appMountMeasurement);
          
          if (config.environment === 'development') {
            console.log(`⚡ アプリケーション初期化完了: ${duration.toFixed(0)}ms`, {
              target: '150ms以下',
              achieved: duration <= 150,
              thresholds: config.performanceThresholds,
        ...
        console.warn('サービス初期化中にエラーが発生しました:', error);
        try {
          const performanceMonitor = PerformanceMonitoringService.getInstance();
          if (appMountMeasurement) {
            performanceMonitor.endMeasurement(appMountMeasurement);
          }
        } catch (e) {
          console.warn('パフォーマンス測定終了に失敗しました:', e);
        }
      }
    };
    ...
    return () => {
      try {
        ...
        console.warn('アプリケーション終了処理に失敗しました:', error);
      }
    };
  }, []);
        ...
        <div className="size-full relative" lang="ja">
          ...
          <Toaster 
            ...
            toastOptions={{
              ...
              className: 'haptic-feedback ultra-fast-transition',
            }}
            closeButton={true}
          />
        </div>
